...
func FindOrCreateFile(fileMd5 string, fileName string, chunkTotal int) (err error, file model.ExaFile) {
	...
	if errors.Is(global.GDb.Where("file_md5 = ? AND is_finish = ?", fileMd5, true).First(&file).Error, gorm.ErrRecordNotFound) {
		err = global.GDb.Where("file_md5 = ? AND file_name = ?", fileMd5, fileName).Preload("ExaFileChunk").FirstOrCreate(&file, cfile).Error
		return err, file
	}
	cfile.IsFinish = true
	cfile.FilePath = file.FilePath
	err = global.GDb.Create(&cfile).Error
	return err, cfile
}
...
func CreateFileChunk(id uint, fileChunkPath string, fileChunkNumber int) error {
	...
	err := global.GDb.Create(&chunk).Error
	return err
}
...
func DeleteFileChunk(fileMd5 string, fileName string, filePath string) error {
	...
	err := global.GDb.Where("file_md5 = ? AND file_name = ?", fileMd5, fileName).First(&file).Update("IsFinish", true).Update("file_path", filePath).Error
	err = global.GDb.Where("exa_file_id = ?", file.ID).Delete(&chunks).Unscoped().Error
	return err
}

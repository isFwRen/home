...
func b0113TempFilterFunc(in, param *pongo2.Value) (*pongo2.Value, *pongo2.Error) {
	//global.GLog.Info("B0113:::MyTempFilterFunc")
	fCode := strings.Split(in.String(), ",")
	val := ""
	var fields []model2.ProjectField
	fmt.Println("B0113reflect.TypeOf(param.Interface()).Name():::", reflect.TypeOf(param.Interface()).Name())
...
func TempRender(export model.SysExport) (err error, tpl *pongo2.Template) {
	if !pongo2.FilterExists("B0113FilterFunc") {
		err = pongo2.RegisterFilter("B0113FilterFunc", b0113TempFilterFunc)
		if err != nil {
			return err, tpl
		}
	}

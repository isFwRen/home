...
func b0108TempFilterFunc(in, param *pongo2.Value) (*pongo2.Value, *pongo2.Error) {
	//global.GLog.Info("B0118:::MyTempFilterFunc")
	fCode := strings.Split(in.String(), ",")
	val := ""
	var fields []model2.ProjectField
	fmt.Println("108reflect.TypeOf(param.Interface()).Name():::", reflect.TypeOf(param.Interface()).Name())
...
func TempRender(export model.SysExport) (err error, tpl *pongo2.Template) {
	if !pongo2.FilterExists("B0108FilterFunc") {
		err = pongo2.RegisterFilter("B0108FilterFunc", b0108TempFilterFunc)
		if err != nil {
			return err, tpl
		}
	}

...
func XmlDeal(o interface{}, xmlValue string) (err error, newXmlValue string) {
	global.GLog.Info("------------------B0121:::XmlDeal-----------------------")
	obj := o.(FormatObj)
	fields := obj.Fields
	...
	c, ok := global.GProConf[obj.Bill.ProCode].ConstTable["B0121_百年人寿团险理赔_百年理赔医院代码表"]
	...
	for ll, lLFeeMainSchema := range lLFeeMainSchemas {
		...
		for _, MedicalBillInfo := range obj.Invoice {
			...
			if fc063 == MainFeeNo {
				...
				_, fc001 = GetOneField(MedicalBillInfo.Fields, "fc001", true)
				_, fc268 = GetOneField(MedicalBillInfo.Fields, "fc268", false)
		...
		if fc001 == "1" && obj.Bill.Agency == "8611" {
			FeeAffixType = "0"
		}
		...
		if fc268 == "A" {
			lLFeeMainSchema = SetNodeValue(lLFeeMainSchema, "FeeDetailType", "1")
		} else if RegIsMatch(fc268, `^(1|2|3)$`) {
			lLFeeMainSchema = SetNodeValue(lLFeeMainSchema, "FeeDetailType", "2")
		}

		FeeDetailType := GetNodeValue(lLFeeMainSchema, "FeeDetailType")
		if FeeDetailType == "2" {
			lLFeeMainSchema = SetNodeValue(lLFeeMainSchema, "FeeDetail", "医保")
		} else if FeeDetailType == "1" {
			lLFeeMainSchema = SetNodeValue(lLFeeMainSchema, "FeeDetail", "普通")
		}
	...
	fc002s := GetSameField(obj.Fields, "fc002", false)
	LLCaseDiseaseInfoSchemas := RegMatchAll(xmlValue, `<LLCaseDiseaseInfoSchema>[\s\S]*?<\/LLCaseDiseaseInfoSchema>`)
	for ii, LLCaseDiseaseInfoSchema := range LLCaseDiseaseInfoSchemas {
		old_LLCaseDiseaseInfoSchema := LLCaseDiseaseInfoSchema
		_, fc349 := GetOneField(obj.Fields, "fc349", true)
		if arrays.Contains(fc002s, "2") != -1 {
			LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "DiagType", "")
			LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "ItemType", "")
			LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "ItemCode", "")
		} else {
			if (arrays.Contains(fc002s, "15") != -1 && ii == 0) || ii >= 2 {
				LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "DiagType", "01")
				LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "ItemType", "1")
				LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "ItemCode", fc349)
			} else if ii == 1 {
				if arrays.Contains(fc002s, "16") != -1 {
					_, fc348 := GetOneField(obj.Fields, "fc348", true)
					LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "ItemCode", fc348)
					LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "DiagType", "06")
					LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "ItemType", "2")
				} else {
					LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "ItemCode", fc349)
					LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "DiagType", "01")
					LLCaseDiseaseInfoSchema = SetNodeValue(LLCaseDiseaseInfoSchema, "ItemType", "1")
		...
		xmlValue = strings.Replace(xmlValue, old_LLCaseDiseaseInfoSchema, LLCaseDiseaseInfoSchema, 1)
	}

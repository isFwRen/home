...
//和发票（通用）有对应关系的：2-发票;3-清单;4-报销单;8-报销单2;9-报销单3;10-报销单4;
//和发票（北京）有对应关系的：14-发票（8611机构、北京发票）;3-清单;4-报销单;8-报销单2;9-报销单3;10-报销单4;
//	fc108发票属性							//MB002-bc005 //MB002-bc018 //MB002-bc016
//  fc109清单所属发票							//MB002-bc006
//  fc118报销单所属发票						//MB002-bc007
//  fc361报销单所属发票						//MB002-bc008
//  fc362报销单所属发票						//MB002-bc009
//  fc363报销单所属发票						//MB002-bc010
...
var relationConf = model2.RelationConf{
	InvoiceFieldCode:          model2.TypeCode{FieldCode: []string{"fc108"}, BlockCode: []string{"^(bc005|bc018|bc016)$"}},
	...
	ThirdBaoXiaoDan1FieldCode: model2.TypeCode{FieldCode: []string{"fc361"}, BlockCode: []string{"bc008"}},
	ThirdBaoXiaoDan2FieldCode: model2.TypeCode{FieldCode: []string{"fc362"}, BlockCode: []string{"bc009"}},
	ThirdBaoXiaoDan3FieldCode: model2.TypeCode{FieldCode: []string{"fc363"}, BlockCode: []string{"bc010"}},
	OtherTempType:             map[string]string{"1": "1", "13": "13", "15": "15", "16": "16"},
	TempTypeField:             "fc002",
...
func ResultData(bill model.ProjectBill, blocks []model3.ProjectBlock, fieldMap map[string][]model3.ProjectField) (err error, obj model2.ResultDataBill) {
	...
	for _, codes := range myCode {
		for _, loc1 := range fieldLocationMap[codes[0]] {
			...
			if !ok {
				//global.GLog.Erro", zap.Error(errors.New("没有匹配全国的常量")))
				continue
			}
	...
	//fc319录入值为1时，fc320结果值为普通
	//fc319录入值为2时，fc320结果值为医保
	//fc319录入值为3时，fc320结果值为生育
	//fc319录入值为4时，fc320结果值为医保审批
	//fc319录入值为5时，fc320结果值为新农合
	myArr := [][]string{
		{"1", "普通"},
		{"2", "医保"},
		{"3", "生育"},
	...
	for _, loc := range fieldLocationMap["fc319"] {
		val := utils.GetFieldValueByLoc(obj, loc, false)
		loc1 := utils.GetFieldLoc(fieldLocationMap["fc320"], loc[0], loc[1], -1, -1)
		for _, item := range myArr {
			if item[0] == val {
				utils.SetFinalValue(obj, loc1, item[1])
			}
		}
	}

	//fc348录入内容匹配常量表《B0121_百年人寿团险理赔_手术代码》的“手术名称”（第三列）对应的“手术代码”（第二列）进行转码；
	for _, loc := range fieldLocationMap["fc348"] {
		val := utils.GetFieldValueByLoc(obj, loc, false)
		utils.SetOnlyOneFinalValue(obj, loc, constMap["shouShuDaiMaMap"][val])
	}

	//fc349录入内容匹配常量表《B0121_百年人寿团险理赔_疾病诊断》的“疾病名称”（第三列）对应的“疾病编码”（第二列）进行转码；
	for _, loc := range fieldLocationMap["fc349"] {
		val := utils.GetFieldValueByLoc(obj, loc, false)
		utils.SetOnlyOneFinalValue(obj, loc, constMap["jiBingZhenDuanMap"][val])
	}

	//fc349录入值为A或为空时，结果值默认为A00
	for _, fc349Loc := range fieldLocationMap["fc349"] {
		fc349LocVal := utils.GetFieldValueByLoc(obj, fc349Loc, false)
		r := regexp.MustCompile("^(A|)$")
		if r.MatchString(fc349LocVal) {
			utils.SetOnlyOneFinalValue(obj, fc349Loc, "A00")
		}
	}
	...
	//fc360为循环分块MB002-bc007报销单中的字段，fc360字段的结果值默认为该分块对应的发票的账单号fc063的结果值
	for _, loc := range fieldLocationMap["fc360"] {
	...
	//fc270,fc272,fc274,fc276,fc278,fc280,fc214,fc215,fc216,fc217,fc218,fc219,fc220,fc221,fc222,fc223,fc242,fc243,fc244,fc245,fc246,fc224,fc225,fc226,fc227,fc228,fc229,fc230,fc231,fc232,fc233,fc252,fc253,fc254,fc255,fc256,fc234,fc235,fc236,fc237,fc238,fc239,fc240,fc241,fc366,fc367,fc368,fc369
	...
	// 20230525新增CSB0121RC0122000
	//1、当fc259的结果值不为空（0、0.00也视为空）时，将fc259的结果值赋值给fc327的结果值
	//2、当fc259的结果值为空（0、0.00也视为空）时，fc327的结果值默认为0  //坐标  //循环
	for _, fc259Loc := range fieldLocationMap["fc259"] {
		fc327Loc := utils.GetFieldLoc(fieldLocationMap["fc327"], fc259Loc[0], -1, -1, -1)
		if len(fc327Loc) < 1 {
			continue
		}
		fc259Val := utils.GetFieldValueByLoc(obj, fc259Loc, true)
		if !utils.RegIsMatch("^(|0|0.00)$", fc259Val) {
			utils.SetFinalValue(obj, fc327Loc, fc259Val)
		} else {
			utils.SetFinalValue(obj, fc327Loc, "0")
		}
	}

	//CSB0121RC0116000
	//fc350结果值默认为（fc322+fc325+fc326）结果值的总和
	myCode = [][]string{
		{"fc350", "fc322", "fc325", "fc326"},
	}
	for _, codes := range myCode {
		for _, loc0 := range fieldLocationMap[codes[0]] {
			loc1 := utils.GetFieldLoc(fieldLocationMap[codes[1]], loc0[0], loc0[1], loc0[2], loc0[4])
			loc2 := utils.GetFieldLoc(fieldLocationMap[codes[2]], loc0[0], loc0[1], loc0[2], loc0[4])
			loc3 := utils.GetFieldLoc(fieldLocationMap[codes[3]], loc0[0], loc0[1], loc0[2], loc0[4])
			if len(loc1) < 1 || len(loc2) < 1 || len(loc3) < 1 {
				continue
			}
			val1 := utils.GetFieldDecimalValueByLoc(obj, loc1[0], true)
			val2 := utils.GetFieldDecimalValueByLoc(obj, loc2[0], true)
			val3 := utils.GetFieldDecimalValueByLoc(obj, loc3[0], true)
			utils.SetOnlyOneFinalValue(obj, loc0, val1.Add(val2).Add(val3).String())
		}
	}

	//CSB0121RC0129000
	//当fc364录入值为1时，清空fc365、fc366、fc367、fc368、fc369、fc370字段的结果值
	codes := []string{"fc365", "fc366", "fc367", "fc368", "fc369", "fc370"}
	for _, loc := range fieldLocationMap["fc364"] {
		val := utils.GetFieldValueByLoc(obj, loc, false)
		if val == "1" {
...
func constDeal(proCode string) map[string]map[string]string {
	...
	nameMap := [][]string{
		{"feiYongDaiMaMap", "B0121_百年人寿团险理赔_百年理赔费用项目代码表", "1", "0"},
		{"yiYuanDaiMaMap", "B0121_百年人寿团险理赔_百年理赔医院代码表", "1", "0"},
		{"shouShuDaiMaMap", "B0121_百年人寿团险理赔_手术代码", "2", "1"},
		{"jiBingZhenDuanMap", "B0121_百年人寿团险理赔_疾病诊断", "2", "1"},
	}
...
func constSpecialDeal(proCode string) map[string]map[string][]string {
	constObj := make(map[string]map[string][]string, 0)
	nameMap := [][]string{
		{"quanGuoMap", "B0121_百年人寿团险理赔_全国", "0"},
	}
	for _, item := range nameMap {
		c, ok := global.GProConf[proCode].ConstTable[item[1]]
		tempMap := make(map[string][]string, 0)

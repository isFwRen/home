...
func CheckXml(o interface{}, xmlValue string) (err error, wrongNote string) {
	global.GLog.Info("B0121:::CheckXml")
	obj := o.(FormatObj)
...
func CodeCheck(obj FormatObj, xmlValue string, constMap map[string]map[string]string) string {
	...
	for _, field := range obj.Fields {
		if RegIsMatch(field.Code, `^(fc349)$`) {
			// fmt.Println("-----------------jibin--------------------", field.Code, field.ResultValue, constMap["jibin"][field.ResultValue])
			_, isexit := constMap["jibin"][field.ResultValue]
			if !isexit {
				wrongNote += "诊断录入有误;"
			}
		}
		if RegIsMatch(field.Code, `^(fc348)$`) {
			// fmt.Println("-----------------shoushu--------------------", field.Code, field.ResultValue, constMap["shoushu"][field.ResultValue])
			_, isexit := constMap["shoushu"][field.ResultValue]
			if !isexit {
				wrongNote += "手术名称录入有误;"
			}
		}
	...
	// fmt.Println("-----------------Agency--------------------", obj.Bill.Agency)
	...
	for _, MedicalBillInfo := range obj.Invoice {
		...
		if fc062 == "B" {
			...
			if fc068 == fc069 {
				...
				wrongNote += "发票号[" + fc063 + "]为住院发票，出入院日期一致，请检查日期或发票类型是否错误;"
			}
		}
		...
		cValue := []string{}
		isWenhao := false
		_, fc365 := GetOneField(MedicalBillInfo.Fields, "fc365", false)
		fc332_names := ""
		for _, field := range MedicalBillInfo.Fields {
			if RegIsMatch(field.Code, `^(fc322|fc323|fc324|fc325|fc326|fc328|fc329|fc330|fc331|fc332|fc333|fc334|fc335|fc336|fc337|fc338|fc339|fc340|fc341|fc342|fc343|fc344|fc345|fc346)$`) && !RegIsMatch(field.FinalValue, `^(\d+|\d+\.?\d+)$`) {
				wrongNote += fc063 + "[" + field.Name + "]字段内容不为金额，请检查;"
			}
			...
			if RegIsMatch(field.Code, `^(fc328)$`) && field.ResultValue != "" {
				wrongNote += fc063 + "发票统筹基金支付金额录入有误;"
			}

			if obj.Bill.Agency == "8611" {
				_, fc326 := GetOneField(MedicalBillInfo.Fields, "fc326", true)
				_, fc325 := GetOneField(MedicalBillInfo.Fields, "fc325", true)
				isHas, fc073 := GetOneField(MedicalBillInfo.Fields, "fc073", true)
				if !isHas {
					continue
				}
				// fmt.Println("-----------------8611--------------------", fc326, fc325, fc073)
				sum := SumFloat(ParseFloat(fc326), ParseFloat(fc325), "+")
				sum = SumFloat(sum, ParseFloat(fc073), "-")
				if sum != 0 && strings.Index(wrongNote, "自费总金额录入有误;") == -1 {
					wrongNote += "发票号[" + fc063 + "]自费总金额录入有误;"
				}
			}
			if fc365 == "1" {
				if RegIsMatch(field.Code, `^(fc366|fc367|fc368|fc369)$`) && field.FinalValue == "" {
					fc332_names += field.Name + "、"
				}
			} else if fc365 == "2" {
				if RegIsMatch(field.Code, `^(fc366|fc367|fc368|fc370|fc072)$`) && field.FinalValue == "" {
					fc332_names += field.Name + "、"
				}
			}
		}
		...
		for _, QingDan := range MedicalBillInfo.QingDan {
			...
			for _, field := range QingDan.Fields {
				if RegIsMatch(field.Code, `^(fc082|fc234|fc090|fc083|fc235|fc091|fc084|fc236|fc092|fc085|fc237|fc093|fc086|fc238|fc094|fc087|fc239|fc095|fc088|fc240|fc096|fc089|fc241|fc097)$`) {
					if strings.Index(field.ResultValue, "?") != -1 && strings.Index(wrongNote, fc063+"票据的项目名称，项目明细分类，项目金额未填写或模糊;") == -1 {
				...
				if is {
					...
					// fmt.Println("------------------------------------------------------", field.ResultValue, fValue, fValue1)
					if (fValue0 != "" || fValue != "" || fValue1 != "") && (fValue0 == "" || fValue == "" || fValue1 == "") && strings.Index(wrongNote, fc063+"票据的项目名称，项目明细分类，项目金额未填写或模糊;") == -1 {
						// fmt.Println("--------------------------ResultValueResultValueResultValue----------------------------", field.Code, fValue0, field.ResultValue, fValue, fValue1)
						wrongNote += fc063 + "票据的项目名称，项目明细分类，项目金额未填写或模糊;"
					}
...
func XmlCheck(obj FormatObj, xmlValue string, constMap map[string]map[string]string) string {
	...
	// c, ok := global.GProConf[obj.Bill.ProCode].ConstTable["B0121_百年人寿团险理赔_百年理赔费用项目代码表"]
	feiyong := constMap["feiyong"]
...
func InitConst(proCode string) map[string]map[string]string {
	...
	nameMap := [][]string{
		{"jibin", "B0121_百年人寿团险理赔_疾病诊断", "2", "2"},
		{"shoushu", "B0121_百年人寿团险理赔_手术代码", "2", "2"},
		{"feiyong", "B0121_百年人寿团险理赔_百年理赔费用项目代码表", "0", "0"},
	}

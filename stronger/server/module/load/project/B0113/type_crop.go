	...
	return global.ProDbMap[proCode+"_task"].Transaction(func(tx *gorm.DB) error {
		for ii, fFields := range opFields {
			...
			for gg, gBlock := range blocks {
				...
				//CSB0113RC0037000
				//根据下载文件压缩包命名规则，直付标记为1的案件，
				//默认设置为紧急单，多次推送或重新加载均需设置为紧急单，直到案件处理完毕再从紧急通道移除
...
func proCustomizeValidation(fields []model.ProjectField, proCode string) error {
	...
	//a.fc002(循环)录入值都不含4、8、9、10时，“理赔类型”的值为“无报销”；
	//b.fc002(循环)录入值包含4、8、9、10，但不包含3时，“理赔类型”的值为“有报销”；
	//c.fc002(循环)录入值同时包含3和4、8、9、10时，“理赔类型”的值为“混合型”；
	//d.不符合以上3点条件的，“理赔类型”的值为空；
	if len(fields) < 1 {
		return errors.New("没有初审字段")
	}
	billId := fields[0].BillID
	val := getFieldsValueByCode(fields, "fc002")
	var claimType = -1
	if !utils.IsContain(val, "4") && !utils.IsContain(val, "8") &&
		!utils.IsContain(val, "9") && !utils.IsContain(val, "10") {
		claimType = 4
	}
	if (utils.IsContain(val, "4") || utils.IsContain(val, "8") ||
		utils.IsContain(val, "9") || utils.IsContain(val, "10")) &&
		!utils.IsContain(val, "3") {
		claimType = 5
	}
	if utils.IsContain(val, "3") && (utils.IsContain(val, "4") || utils.IsContain(val, "8") ||
		utils.IsContain(val, "9") || utils.IsContain(val, "10")) {
		claimType = 6
	}

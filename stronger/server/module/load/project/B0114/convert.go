...
func Convert(proCode string, projectBill model.ProjectBill) (error, model.ProjectBill) {
	...
	for ii, picture := range pictures {
		...
		if global.GConfig.System.Env == "test" {
			url = "http://192.168.3.89:18100/detect"
		}
		...
		objects := make(map[string][]OCRResp)
		err = json.Unmarshal([]byte(stdout), &objects)
		if err != nil {
			global.GLog.Error("classifier-json-Unmarshal-err", zap.Error(err))
		}
		label := make([]string, 0)
		global.GLog.Info("object", zap.Any("s", objects["objects"]))
		for _, object := range objects["objects"] {
			labelArr := strings.Split(object.Label, "/")
			labels := strings.Split(labelArr[len(labelArr)-1], "、")
			label = append(label, labels[len(labels)-1])
		}
		projectBill.ImagesType[ii] = strings.Join(label, "、")

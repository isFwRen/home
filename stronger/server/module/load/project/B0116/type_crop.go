...
func proCustomizeValidation(fields []model.ProjectField, proCode string) error {
	...
	// 	初审录入完毕后，对所有fc086进行校验：
	// 1.当所有fc086的录入值包含“4”且不包含“5”时，案件类型默认为：无报销
	// 2.当所有fc086的录入值包含“5”且不包含“4”时，案件类型默认为：有报销
	// 3.当所有fc086的录入值包含“4”且包含“5”时，案件类型默认为：混合型
	// 4.当所有fc086的录入值不包含“4”且不包含“5”时，案件类型默认为：空
	if len(fields) < 1 {
		return errors.New("没有初审字段")
	}
	billId := fields[0].BillID
	val := getFieldsValueByCode(fields, "fc086")
	var claimType = -1
...
func constDeal(proCode string) map[string]map[string]string {
	...
	nameMap := [][]string{
		{"diZhiShengShiQuMap", "B0116_华夏理赔_华夏理赔地址库", "4", "1"},
		{"jiGouBianMaMap", "B0116_华夏理赔_机构编码对应表", "1", "0"},
	}
	...
	for k, v := range global.GProConf[proCode].ConstTable {
		if strings.Index(k, "省份") != -1 {
			...
			constObj[strings.Replace(k, "B0116_华夏理赔_省份-", "", -1)] = tempMap
		}
	}
...
func specialConst(proCode string) map[string]map[string][]string {
	constObj := make(map[string]map[string][]string, 0)
	nameMap := [][]string{
		{"diZhiMap", "B0116_华夏理赔_华夏理赔地址库", "4", "0"},
		{"diZhiShiMap", "B0116_华夏理赔_华夏理赔地址库", "3", "0"},
		{"diZhiShengMap", "B0116_华夏理赔_华夏理赔地址库", "2", "0"},
	}

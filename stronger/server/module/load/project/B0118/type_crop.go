...
func proCustomizeValidation(fields []model.ProjectField, proCode string) error {
	//初审录入完毕后，对所有fc056进行校验：
	//1.当所有fc056的录入值包含“3”且不包含“4”时，理赔类型默认为：非医疗
	//2.当所有fc056的录入值包含“3、4”时，理赔类型默认为：医疗
	//d.不符合以上2点条件的，“理赔类型”的值为空；
	if len(fields) < 1 {
	...
	if utils.IsContain(val, "3") && !utils.IsContain(val, "4") {
		claimType = 4
	}
	if utils.IsContain(val, "3") && utils.IsContain(val, "4") {
		claimType = 6

...
func FetchBill(projectBill model.ProjectBill) (error, model.ProjectBill) {
	...
	localFile0 := global.GConfig.LocalUpload.FilePath + projectBill.DownloadPath + "/" + projectBill.Files[0]
	localFile1 := global.GConfig.LocalUpload.FilePath + projectBill.DownloadPath + "/" + projectBill.Files[1]
	remoteFile0 := projectBill.BillName + "/" + projectBill.Files[0]
	remoteFile1 := projectBill.BillName + "/" + projectBill.Files[1]
	cmd := fmt.Sprintf(global.GProConf[global.GConfig.System.ProCode].DownloadPaths.FetchBill, localFile0, remoteFile0, localFile1, remoteFile1)

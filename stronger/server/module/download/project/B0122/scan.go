...
func Scan() []model.ProjectBill {
	...
	global.GLog.Info("B0122-download", zap.Any("scan ", stdout))
	re := regexp.MustCompile("[\r\n]+")
	lines := re.Split(stdout, -1)
	global.GLog.Info("B0122-download", zap.Any("lines ", lines))
	lineReg := regexp.MustCompile(`(\S+)\s+(\S+\s+\S+\s+\S+)\s+(\S+)$`)
	for ii, line := range lines {
		global.GLog.Info("B0122-download", zap.Any(line, ii))
		if line == "" {
			continue
		}
		matched, _ := regexp.MatchString(`.zip$`, line)
		arr := lineReg.FindStringSubmatch(line)
		global.GLog.Info("B0122-download", zap.Any("arr", arr))
		if matched && len(arr) >= 4 {
			global.GLog.Info("B0122-download", zap.Any("arr[3]", arr[3]))
			...
			bDay := time.Now().AddDate(0, -1, 0)
			fmt.Println("--------------parse----------------", parse, bDay)
			if parse.Before(bDay) {
				fmt.Println("--------------delete----------------", bDay)
				Clean(projectBill)
				continue
			}

...
func Decompress(projectBill model.ProjectBill) (error, model.ProjectBill) {
	...
	claim_id := utils.GetNodeValue(data, "claim_id")
	claim_type := utils.GetNodeValue(data, "claim_type")
	...
	projectBill.InsuranceType = mapType[claim_type]
	...
	projectBill.BillNum = claim_id // utils.Substr(projectBill.BillName, 0, 20)
	...
	// c, ok := global.GProConf["B0122"].ConstTable["B0122_信诚理赔_机构号代码表"]
	// if ok {
	// 	for _, arr := range c {

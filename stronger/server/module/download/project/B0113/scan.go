...
func Scan() []model.ProjectBill {
	...
	for ii, line := range lines {
		...
		if len(arr) >= 4 {
			...
			mark := zipNameArr[len(zipNameArr)-1]

			//CSB0113RC0109000
			//案件列表-销售渠道，显示每个案件直付标记的值所对应的中文
			//0-核心；1-直付；2-微信；3-保医
			//下载文件命名格式：
			//批次号_该批次下的总案件数_赔案号_该案件在该批次下的顺序号_机构号_直付标记.zip
			var markMap = map[string]string{"0": "核心", "1": "直付", "2": "微信", "3": "保医"}
			projectBill.SaleChannel = markMap[mark]

			re = regexp.MustCompile("^8060")
			if !re.MatchString(projectBill.BillNum) {
				global.GLog.Error("该单据不是个险：" + projectBill.BillName)
				continue
			}

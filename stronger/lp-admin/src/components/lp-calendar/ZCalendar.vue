		...
		<div class="calendar-box">
			...
			<div class="z-flex justify-center pb-2 calendar-box-header">
				...
				<div class="month">
					<div class="pl-4 pr-1 prev-month">
						<v-btn icon @click="onPrevMonth(), onSelect('prev')">
					...
					<div class="pl-1 pr-4 next-month">
						<v-btn icon @click="onNextMonth(), onSelect('next')">
				...
				<div class="back-today">
					<v-btn depressed @click="onBackToday(), onSelect('today')"> 返回今天 </v-btn>
				</div>
			</div>
			<!--  -->
			...
			<div class="calendar-box-content">
				<div class="content-header">
					<div v-for="item in weekName" :key="item.value" class="week">
						{{ item.label }}
					</div>
				</div>
...
export default {
	...
	methods: {
		...
		_setSelectedItems(value) {
			...
			if (selected) {
				const isRepeat = this.selectedItems.find(

				...
				<v-row no-gutters style="height: 50px" align="center">
					<v-col cols="11">
						<div class="header-title">
							...
							<h5 class="text-lg-h6" style="font-size: 16px !important">身份认证</h5>
						</div>
					</v-col>
				...
				<v-row align="center" justify="center" no-gutters style="margin-top: 15px">
					<v-col cols="8">
						<div class="authen-forms">
							<v-form ref="entryForm">
								...
								<div class="forms-item">
									<v-text-field :label="passwordField.label" @click:append="eye = !eye" :type="eye ? 'text' : 'password'"
										:append-icon="eye ? 'mdi-eye' : 'mdi-eye-off'" v-model="forms.password" :rules="passwordField.rules">
									</v-text-field>
								</div>
					...
					<v-col>
						...
						<div class="code-img-box" v-else>
							...
							<p class="code-desc">
								使用验证码app扫描上方的二维码获取登录验证码后关闭弹窗，使用该验证码进行登录
							</p>
						</div>
					</v-col>
				</v-row>
...
export default {
		...
		"forms.phone": {
			handler() {
				if (this.forms["phone"]) {
					...
					this.validAccount = this.networkIdetify
						? !!phone
						: phone && regex_phone.test(phone);
				}
			},
			immediate: true
		}
	...
	methods: {
		async authFun() {
			...
			const result = await this.$store.dispatch("VERIFY_IDENTITY", form);
			if (result.code === 200) {
...
<style lang="scss">
@mixin position_box($position: null,
	$left: null,
	$top: null,
	$right: null,
	$bottom: null,

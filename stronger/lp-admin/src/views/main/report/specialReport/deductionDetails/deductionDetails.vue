		...
		<div class="z-flex align-end mb-8 lp-filters">
			<v-row class="z-flex align-end">
				...
				<div class="z-flex">
					...
					<z-btn :formId="formId" class="pb-3 pl-3" color="error" @click="diffStr">
						复制
					</z-btn>
					<z-btn :formId="formId" class="pb-3 pl-3" color="success" @click="onExport">
						导出
					</z-btn>
				</div>
			</v-row>
...
export default {
	...
	data() {
		return {
			cells,
			dispatchList: "GET_DEDUCTION_DETAILS_LIST",
			formId: "deductionDetails"
		};
	},
	computed: {
		...mapState(["forms"])
	...
	methods: {
		...
		async onExport() {
			...
			lpTools.createExcelFun(
				result,
				`${form.proCode}扣费明细${form.time[0]}-${form.time[1]}`
			);

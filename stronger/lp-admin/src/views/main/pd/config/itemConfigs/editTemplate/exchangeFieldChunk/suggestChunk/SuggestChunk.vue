<template>
	<div class="suggest-chunk">
		<v-row class="exchange-container">
			<v-col cols="6">
				<h6 class="text-h6 fw-bold title">关联分块</h6>
						...
						>
							<transition-group type="transition" :name="!drag ? 'flip-list' : null">
								<div
									...
									v-for="(item, index) in suggestChunkList"
									:key="index"
								>
									<v-chip
...
export default {
	...
	methods: {
		async saveExchange() {
			...
			if (this.suggestChunkList.length) {
				for (let item of this.suggestChunkList) {
	...
	watch: {
		"editTemp.suggestChunkList": {
			handler(list) {
				this.suggestChunkList = [...list];
			},
			immediate: true
		},

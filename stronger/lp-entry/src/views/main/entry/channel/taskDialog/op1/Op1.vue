<template>
	<div v-if="hasOp" class="op op1op2opq">
		<v-row class="full ma-0">
			...
			<v-col id="op1FieldsNode" class="full fields-wrapper" cols="3" ref="col">
				...
				<v-form ref="op1Form" v-model="valid" lazy-validation @submit="preventForm">
...
export default {
	...
	mounted() {
		let tLoadingNode = document.querySelector(".t-loading");
	},
		...
		focusFieldIndex: {
			handler(index, oldValue) {
				if (oldValue !== undefined && index !== oldValue) {
					...
					// console.log("下一输入框---------");
					// console.log("this.firstDiffIndex", this.firstDiffIndex);
					// console.log("this.lastDiffIndex", this.lastDiffIndex);
					// console.log("this.indexsIndex", this.indexsIndex);
				}
			},
			immediate: true
		}
	...
	methods: {
		...
		opqMarkFieldFirstDiffIndex(field, symbol = "?") {
			if (field[`${this.op}Value`]) {
				...
				// console.log("this.firstDiffIndex-----下----", this.firstDiffIndex);
				// console.log("this.lastDiffIndex-----下------", this.lastDiffIndex);
			} else {
				this.firstDiffIndex = -1;
			}
		},

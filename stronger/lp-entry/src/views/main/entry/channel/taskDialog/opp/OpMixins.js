...
export default {
    ...
    computedLabel() {
      ...
      if (env === 'development' || env === 'test') {
        return function ({ field, fieldIndex, fieldsIndex }) {
          ...
          return `(${code}_${fieldsIndex}-${fieldIndex})${name}`
        }
      }
  ...
  methods: {
    ...
    validateField({ field }) {
      ...
      if (field?.rules && field?.rules[index]?.rule.includes(field.op1Value)) return true
    ...
    async validateFields(fields, key) {
      ...
      for (let field of fields) {
        ...
        const { nValidations, nvArgs } = field.checkMethod
        let common
        // 普通校验
        const nVerified = nValidations.every(validation => {
          const result = validators[validation['key']]({ ...nvArgs(validation), effectValidations: validation.effectValidations })
        ...
        if ((typeof common) == 'string') {
          ...
          if (sessionStorage.get('isApp')?.isApp === 'true') {
            await MessageBox.confirm(`禁止${key}，请修改或删除第${page}张图的${field.code}_${fieldName}${field.name}的内容, 禁止原因：内容${field.resultValue}不满足条件${common}`, '请检查', {
              type: 'warning',
            ...
            result = alert(`禁止${key}，请修改或删除第${page}张图的${field.code}_${fieldName}${field.name}的内容, 禁止原因：内容${field.resultValue}不满足条件${common},`)
          }

	...
	<div class="lp-error">
		<div class="mb-8 lp-filters">
			<v-row class="z-flex align-end">
				...
				<v-col :cols="2">
					<z-text-field :formId="searchFormId" formKey="fieldName" hideDetails label="字段名称"> </z-text-field>
				</v-col>
		...
		<div class="table error-detail-table">
			...
			>
				...
				<template v-for="item in cells.headers">
					<!-- 错误数据 BEGIN -->
					<vxe-column v-if="item.value === 'wrong'" :field="item.value" :title="item.text" :key="item.value">
					...
					<!-- 申诉 BEGIN -->
					<vxe-column
						v-else-if="item.value === 'isComplain'"
						:field="item.value"
						:title="item.text"
						:key="item.value"
						:width="item.width"
					>
						<template #default="{ row, rowIndex }">
							<z-switch
								:formId="formId"
								:formKey="`appeal_${rowIndex}_${row.id}`"
								:disabled="row.isComplain"
								:label="row.isComplain ? '已申诉' : '申诉'"
								:defaultValue="row.isComplain"
								@change="onAppeal($event, row)"
							>
							</z-switch>
						</template>
					</vxe-column>
...
export default {
	...
	methods: {
		...
		async onAppeal(value, row) {
			const reg = /^[\?？]+$/g;
			if (reg.test(row.wrong)) {
				this.toasted.dynamic("纯问号内容，不计入正确率统计，无需进行申诉", 400);
			} else {
				const body = {
					proCode: this.forms[this.searchFormId].proCode,

          ...
          >
            <template v-for="(item) in headers">
              ...
              <div class="py-2 z-flex">
                {{(item.output&&item.output(rowIndex))||row[item.value]}}
              </div>
            </template>
              </vxe-column>
            </template>
  ...
  export default {
    ...
    data() {
      return {
        formId: 'homeRanking',
        headers: [
          { text: '名次', value: 'myOrder' ,output:(e)=>{
            return (this.params.pageIndex - 1) * this.params.pageSize + e + 1;
          }},

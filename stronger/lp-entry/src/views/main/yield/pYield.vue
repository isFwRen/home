	...
	<div class="lp-error">
		<div class="mb-8 lp-filters">
			<v-row class="z-flex align-end">
				...
				<v-col :cols="2">
					<z-text-field :formId="searchFormId" formKey="code" clearable hideDetails label="工号"></z-text-field>
				</v-col>

				<v-col :cols="2">
					<z-text-field :formId="searchFormId" formKey="name" clearable hideDetails label="姓名"></z-text-field>
				</v-col>
		...
		<div class="table error-detail-table">
			...
			>
				...
				<template v-for="item in cells.pheaders">
					<vxe-column v-if="item.value === 'applyAt'" :field="item.value" :title="item.text" :key="item.value">
						<template #default="{ row }">
							<span>{{ row.applyAt.slice(0, 10) }}</span>
						</template>
					</vxe-column>
					<vxe-column v-else-if="item.value === 'submitAt'" :field="item.value" :title="item.text" :key="item.value">
						<template #default="{ row }">
							<span>{{ row.submitAt.slice(0, 10) }}</span>
						</template>
					</vxe-column>

					<vxe-column
						v-else-if="item.value === 'summaryAccuracyRate'"
						:field="item.value"
						:title="item.text"
						:key="item.value"
					>
						<template #default="{ row }">
							<span>{{ row.summaryAccuracyRate * 100 + "%" }}</span>
						</template>
					</vxe-column>
					<vxe-column v-else-if="item.value === 'search'" :field="item.value" :title="item.text" :key="item.value">
						<template>
							<span class="primary--text" @click="$router.push('/main/error/practice')">查看</span>
						</template>
					</vxe-column>

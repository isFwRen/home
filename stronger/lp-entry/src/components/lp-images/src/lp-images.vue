...
<script>
import EventMixins from "./EventMixins";
...
export default {
	...
	watch: {
		src: {
			handler(src) {
				this.resetValues();
				this.overlay = true;
				lpTools.loadImage(src, this.setImage);
			},
			immediate: true
		}
	},
	...
	methods: {
		...
		transformImage() {
			...
			// }
			this.imageWrap = this.imageWrap ?? document.querySelector("#view .image-wrap");
